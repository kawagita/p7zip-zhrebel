...
static const char *kMethod_Unknown = "Unknown";
...
STDMETHODIMP CHandler::GetProperty(UInt32 index, PROPID propID, PROPVARIANT *value)
{
  ...
  switch (propID)
  {
    case kpidPath:
    {
      UString res;
      #ifdef ZIP_HEADER_REBEL
      if (m_HeaderLocale.HasDecodingCharset() && !m_HeaderLocale.SwitchDecodingLocale())
        return E_LANG_NOT_FOUND;
      #endif
    ...
    case kpidExtra:
    {
      ...
      FOR_VECTOR (i, subBlocks)
      {
        ...
        else if (id == NFileHeader::NExtraID::kUnixTime)
          m += "UT";
        else if (id == NFileHeader::NExtraID::kWzAES)
          m += "WzAES";
        else
        {
          m += "0x";
    ...
    case kpidComment:
    {
      if (item.Comment.Size() != 0)
      {
        UString res;
        #ifdef ZIP_HEADER_REBEL
        if (m_HeaderLocale.HasDecodingCharset(true) && !m_HeaderLocale.SwitchDecodingLocale(true))
          return E_LANG_NOT_FOUND;
        #endif
