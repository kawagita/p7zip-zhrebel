    ...
    UInt64 &complexity)
{
  ...
  if (ui.NewProps)
  {
    ...
    unsigned localExtraSize = 0;
    #ifdef ZIP_HEADER_REBEL
    headerRebel->SetLocalHeaderLocale(item);
    headerRebel->ChangeLocalHeader(item, *extraStorage);
    localExtraSize = extraStorage->GetLocalExtraSize(item);
    #endif
    archive.PrepareWriteLocalHeaderData(localExtraSize);
    archive.PrepareWriteCompressedData2(item.Name.Len(), item.Size, item.PackSize, item.LocalExtra.HasWzAes());
    #ifdef ZIP_HEADER_REBEL
    extraStorage->RestoreExtraSubBlocks(item);
    extraStorage->RestoreExtraWzAes(item);
    #endif
  ...
  FOR_VECTOR (itemIndex, updateItems)
  {
    ...
    if (ui.IndexInArc >= 0)
    {
      ...
      if (inArchive->ReadLocalItemAfterCdItemFull(itemEx) != S_OK)
        return E_NOTIMPL;
      else if (!ui.NewProps || !ui.NewData)
        (CItem &)item = itemEx;
      #ifdef ZIP_HEADER_REBEL
      headerRebel->BackupHeaderExtra(itemEx, extraStorage);
      #endif
    }
    ...
    if (ui.NewData)
    {
      ...
      if (isDir)
      {
        WriteDirHeader(archive, options,
                       #ifdef ZIP_HEADER_REBEL
                       headerRebel,
                       &extraStorage,
                       #endif
    ...
    else
    {
      UInt64 complexity = 0;
      lps->SendRatio = false;
      RINOK(UpdateItemOldData(archive, inArchive,
            #ifdef ZIP_HEADER_REBEL
            headerRebel,
            &extraStorage,
            #endif
            itemEx, ui, item, progress, opCallback, complexity));
      lps->SendRatio = true;
    ...
    IArchiveUpdateCallback *updateCallback)
{
  ...
  if (!mtMode)
    ...
    return Update2St(
        EXTERNAL_CODECS_LOC_VARS
        archive, inArchive,
        inputItems, updateItems, &options2, comment,
        #ifdef ZIP_HEADER_REBEL
        headerRebel,
        #endif
        updateCallback, totalComplexity, opCallback);
