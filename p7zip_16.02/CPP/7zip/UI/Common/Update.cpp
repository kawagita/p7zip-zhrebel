    ...
    CFinishArchiveStat &st)
{
  ...
  RINOK(SetHeaderProperties(outArchive, options.MethodMode.HeaderProperties));
    ...
    bool needSetPath)
{
  ...
  else
  {
    ...
    {
      if (renameMode || changeHeaderOnly)
    ...
    else
    {
      ...
      op.headerProps = &options.MethodMode.HeaderProperties;
      #endif
      op.codecs = codecs;
      op.types = &types2;
  ...
  if (!thereIsInArchive && (renameMode || changeHeaderOnly))
    return E_FAIL;
