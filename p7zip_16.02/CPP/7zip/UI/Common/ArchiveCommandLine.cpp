...
enum Enum
{
  ...
  kUpdate,
  kUpdateDirectoryHeaderOnly,
  kUpdateFileHeaderOnly,
  kVolume,
  kRecursed,
...
bool CArcCommand::IsFromUpdateGroup() const
{
  switch (CommandType)
  {
    ...
    case NCommandType::kUpdateHeaderOnly:
...
static bool ParseArchiveCommand(const UString &commandString, CArcCommand &command)
{
  ...
  if (s.Len() == 2)
  {
    ...
    {
      command.CommandType = (NCommandType::kUpdateHeaderOnly);
      return true;
    }
  }
  return false;
...
static void AddToCensorFromNonSwitchesStrings(
    ...
    bool wildcardMatching,
    bool updateHeaderOnly,
    bool thereAreSwitchIncludes, Int32 codePage)
{
  if ((renamePairs || nonSwitchStrings.Size() == startIndex) && !updateHeaderOnly && !thereAreSwitchIncludes)
    ...
    CUpdateOptions &options)
{
  ...
  switch (commandType)
  {
    ...
    case NCommandType::kDelete:
      ...
      break;
    case NCommandType::kUpdateHeaderOnly:
      defaultActionSet = NUpdateArchive::k_ActionSet_UpdateHeaderOnly;
      break;
    default:
  ...
  {
    if (commandType == NCommandType::kUpdateHeaderOnly)
      throw CArcCmdLineException("Update options cannot be used to change header only");
...
{
  if (parser[NKey::kHeaderProperty].ThereIs)
  {
    CHeaderPropertyParser *propParser =
        new CHeaderPropertyParser(kHeaderPropertyForms, ARRAY_SIZE(kHeaderPropertyForms), &properties);
    FOR_VECTOR (i, parser[NKey::kHeaderProperty].PostStrings)
    {
      CProperty prop;
      prop.Name = parser[NKey::kHeaderProperty].PostStrings[i];
      int index = prop.Name.Find(L'=');
      if (index >= 0)
      {
        prop.Value = prop.Name.Ptr(index + 1);
...
void CArcCmdLineParser::Parse2(CArcCmdLineOptions &options)
{
  ...
  bool isUpdateHeaderOnly = options.Command.CommandType == NCommandType::kUpdateHeaderOnly;
  ...
  AddToCensorFromNonSwitchesStrings(isRename ? &options.UpdateOptions.RenamePairs : NULL,
      ...
      nonSwitchStrings, recursedType, wildcardMatching, isUpdateHeaderOnly,
  ...
  SetHeaderOptions(parser, options.HeaderProperties);
  ...
  else if (options.Command.IsFromUpdateGroup())
  {
    ...
    updateOptions.MethodMode.HeaderProperties = options.HeaderProperties;
