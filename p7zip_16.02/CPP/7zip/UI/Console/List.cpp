...
static void PrintTime(char *dest, const FILETIME *ft, bool printNanoSeconds = false)
{
  *dest = 0;
  ...
  if (!FileTimeToLocalFileTime(ft, &locTime))
    throw 20121211;
  ConvertFileTimeToString(locTime, dest, true, true, printNanoSeconds);
}
  ...
  */
  FOR_VECTOR (i, _fields)
  {
    ...
    else
    {
      ...
      {
        PrintTime(temp + tempPos, &prop.filetime, techMode);
        if (techMode)
          g_StdOut << temp + tempPos;
        else
        {
...
HRESULT ListArchives(CCodecs *codecs,
    ...
    const CObjectVector<CHeaderProperty> *headerProps,
    #endif
    UInt64 &numErrors,
    UInt64 &numWarnings)
{
...
*/
    ...
    */
    ...
    #ifndef _SFX
    options.props = props;
    options.headerProps = headerProps;
    #endif
