...
7za-zhrebelコマンドはファイル種別を変更できませんが、ZIPアーカイブの（指定もできる）作成環境に応じて下表にあるデータを変更できます。足されたビットのファイル属性やパーミッションには角括弧内のアルファベットやrwxの文字が`l`コマンドで表示されます。

Lower two bytes:

| Bit allocation   | Value                 |                                          |
| :--------------- | :-------------------- | :--------------------------------------- |
...
|    ......        |                       |                                          |
| 0010000000000000 | Not Content [I]ndexed | インデックス付きでない事を示す           |
|    ......        |                       |                                          |
| 1000000000000000 | UNIX Extension        | UNIX延長？（7zaが付けるフラグ）          |

Upper two bytes:

| Bit allocation   | Value               |                                    |
| :--------------- | :------------------ | :--------------------------------- |
| 0000000000000111 | Others permissions  | その他のファイルへのアクセス許可   |
| 0000000000111000 | Group's permissions | グループのファイルへのアクセス許可 |
| 0000000111000000 | Owner's permissions | 所有者のファイルへのアクセス許可   |
|    ......        |                     |                                    |
